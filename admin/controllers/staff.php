...
class SirsControllerStaff extends JController
{
	/**
	 * Constructor
	 */
	...
	{ 
			...
			if (!$user->bind($post))
			{
				...
				$mainframe->redirect( 'index.php?option=com_sirs&c=staff', $user->getError() );
				return false;
			...
			//insert user_group
			$ESql = "INSERT INTO #__user_usergroup_map (`user_id`, `group_id`) VALUES ('".$post['user_id']."','6')";
			$db->setQuery($ESql);
			$RESql = $db->query();
			...
			$row->checkin();
			$link = 'index.php?option=com_sirs&c=staff';
	...
	function cancel()
	{
		...
		$this->setRedirect( 'index.php?option=com_sirs&c=staff', $msg );
	}
	...
	function remove()
	{
		...
		$getUser = "SELECT a.id FROM #__users AS a LEFT JOIN #__sirs_staff AS b ON (b.user_id=a.id) WHERE b.id IN (".$cid.")";
		$db->setQuery($getUser);
		$RgetUser = $db->loadObjectList();
		
		$deleteMember = "DELETE FROM #__sirs_staff WHERE id IN (".$cid.")";
		...
		$this->setRedirect( 'index.php?option=com_sirs&c=staff' ); 
