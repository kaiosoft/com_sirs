...
class SirsControllerJabatan extends JController
{
	/**
	 * Constructor
	 */
	...
	/**
	 * Display the list of jabatan
	 */
	function display()
	{
		JRequest::setVar('view', 'jabatan' );
	...
	{ 
		...
		$row =& JTable::getInstance('jabatan', 'Table');
		...
		$sql = "SELECT * FROM #__sirs_jabatan WHERE jabatan='".$post['jabatan']."'";
		$db->setQuery($sql);
		$res = $db->loadResult();
		...
		if ($row->store($post)) {
			$msg = JText::_( 'Data jabatan Telah Di Simpan' );
		} else {
		...
		$row->checkin();
		$link = 'index.php?option=com_sirs&c=jabatan';
	...
	function cancel()
	{
		...
		$this->setRedirect( 'index.php?option=com_sirs&c=jabatan', $msg );
	}
	...
	function remove()
	{
		...
		$cid = implode(',', $cid);		
		$q = 'DELETE FROM #__sirs_jabatan WHERE id IN ('.$cid.')';
		$db->setQuery( $q );
		...
		$this->setRedirect( 'index.php?option=com_sirs&c=jabatan' ); 
